...
function avoidVariant(item: DailyBriefTopicToAvoid): BadgeProps["variant"] {
  ...
  if (item.severity === "saturated" || item.severity === "stale")
    return "accent";
  return "muted";
}
...
function TrendMiniCard({
  ...
  onOpen: (trend: DashboardTrend) => void;
}) {
  return (
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge
              variant={trend.status === "Hidden Gem" ? "secondary" : "accent"}
            >
              {trend.status}
            </Badge>
          </div>
          <p className="mt-2 line-clamp-2 text-sm leading-6 text-muted-foreground/76">
            {trend.summary}
          </p>
          <p className="mt-2 text-xs text-muted-foreground/60">
            Score {trend.trendScore} · Hidden gem {trend.hiddenGemScore} ·{" "}
...
export function DailyBriefView() {
  ...
  const [selectedTrend, setSelectedTrend] = useState<DashboardTrend | null>(
    null,
  );
  ...
  const reportDocument = useMemo(() => {
    ...
    return buildTemplateReportDocument(
      brief.reportDocument,
  ...
  const loadBrief = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !payload.ok) {
        throw new Error(
          readApiErrorMessage(payload, "Daily Brief could not be loaded."),
        );
      }
      ...
      setError(
        loadError instanceof Error
          ? loadError.message
          : "Daily Brief could not be loaded.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const markdown = useMemo(() => {
    ...
    return (
      reportDocument.quickCopy.markdown ||
      buildTemplateMarkdown(reportDocument, preferences.reportTemplate)
    );
    ...
    return [
      ...
      },
      {
        ...
        value:
          ...
          brief.intelligenceNarratives.find(
            (item) => item.id === "market-posture",
          )?.verdict ??
      ...
      },
      {
        ...
        value:
          (topAction?.trend ?? bestResearch ?? bestGem)?.productIntelligence
            .startupAngle ??
    ...
    <AppShell>
      <div className="space-y-5 sm:space-y-6">
        ...
        <section className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-sm leading-6 text-muted-foreground/78 md:text-base">
              A simplified decision brief for what to act on, watch, turn into
        ...
        {isMissingDatabaseConfig ? (
          <FirstRunStateCard onRetry={() => void loadBrief()} />
        ) : error ? (
          <ProductStateCard
            variant="error"
            ...
            description={error}
            action={
              <Button type="button" size="sm" onClick={() => void loadBrief()}>
                Try again
              </Button>
            }
          />
        ...
        {hasNoScanData ? (
          <NoScanStateCard
            title="Daily Brief is waiting for the first scan"
            description="The report model loaded, but there are no stored trend signals yet. Run Scan Trends Now from the dashboard before using this page for decisions."
          />
        ) : null}
          ...
          <>
            ...
            <section className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">
              <MetricCard
                ...
                helper="Items strong enough to act on now."
              />
              <MetricCard
                label="Hidden gems"
                value={brief.radarStats.hiddenGems}
                helper="Early signals with room before saturation."
              />
              <MetricCard
                label="Creator opportunities"
                value={brief.radarStats.creatorOpportunities}
                helper="Topics with content timing upside."
              />
              <MetricCard
                label="Research signals"
                value={brief.radarStats.researchSignals}
                helper="arXiv-backed candidates to validate."
              />
              <MetricCard
                label="Avoid"
                value={brief.radarStats.topicsToAvoid}
                helper="Noisy, weak or over-saturated topics."
              />
            </section>
            ...
            <Card className="border-secondary/15 bg-[#160d0d]/62">
              <CardHeader>
                ...
                <CardTitle>
                  What matters now, not every metric the system knows.
                </CardTitle>
                <CardDescription>
                  A product-facing readout shaped by source quality, trend
            ...
            {preferences.reportTemplate === "pitch" ||
            preferences.experienceMode === "pitch" ? (
            ...
            {brief.topPriorityActions.length > 0 ? (
              <Card className="border-border/10 bg-[#160d0d]/62">
                <CardHeader>
                  ...
                  <CardDescription>
                    Only topics with enough confirmation. Anything uncertain is
                    pushed into watch or research first.
                  </CardDescription>
                </CardHeader>
                      ...
                      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Badge variant={actionVariant(item.actionPriority)}>
                              {actionPriorityText(item.actionPriority)}
                            </Badge>
                            <Badge variant="muted">
                        ...
                        <Button
                          ...
                          variant="ghost"
                          onClick={() => {
                            setSelectedTrend(item.trend);
                            setSelectedSlug(item.trend.slug);
                          }}
                        >
                    ...
                    brief.hiddenGemsWorthWatching.slice(0, 5).map((trend) => (
                      <TrendMiniCard
                        ...
                        onOpen={(nextTrend) => {
                          ...
                          setSelectedSlug(nextTrend.slug);
                        }}
                      />
                    ))
                  ) : (
                    <p className="text-sm text-muted-foreground/72">
                      No strong hidden gems in this window.
                    </p>
                  )}
                </CardContent>
              </Card>
            ...
            {brief.topicsToAvoid.length > 0 ? (
              <Card className="border-primary/15 bg-primary/10">
                <CardHeader>
                  ...
                  <CardDescription>
                    These topics look noisy, weak, stale or too saturated for
                    clean action.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  {brief.topicsToAvoid.slice(0, 5).map((item) => (
                    ...
                    >
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge variant={avoidVariant(item)}>
